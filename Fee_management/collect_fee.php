...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($result && mysqli_num_rows($result) > 0) {
      $_SESSION['student_id'] = $student_id;
        header("Location: collect_fee_2.php");
        exit;
    } else {
...
.back-btn {
  ...
  margin-top: 15px;
  padding: 12px;
  background-color: #00d9ff; /* Same as search button */
  border: none;
  color: #0f172a; /* Same text color as search button */
  font-size: 16px;
  border-radius: 8px; /* Same as search button */
  font-weight: 600;
  letter-spacing: 0.5px;
  cursor: pointer;
